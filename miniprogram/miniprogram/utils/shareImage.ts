...
const shareImagePromiseCache: Record<string, Promise<string>> = {};
const shareImageResultCache: Record<string, string> = {};

function buildShareImageCacheKey(sourceUrl: string): string {
  let hash = 2166136261;
...
function normalizeShareSourceUrl(sourceUrl: string): string {
  return normalizeCosUrl(String(sourceUrl || '').trim());
}
...
function ensureDirectory(dirPath: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    fs.mkdir({
      ...
      success: () => resolve(),
      fail: (error) => {
        const errorMessage = String((error as any)?.errMsg || '');
...
function canvasToDataURL(canvas: any): Promise<string> {
  return new Promise((resolve, reject) => {
    canvas.toDataURL({
      type: 'image/jpeg',
      quality: 0.92,
      success: (res: any) => resolve(String(res?.data || '').trim()),
      fail: reject,
    });
  });
}

function writeBase64Image(cacheKey: string, dataUrl: string): Promise<string> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const base64ToArrayBuffer = (wx as any).base64ToArrayBuffer;
...
function computeContainSize(srcWidth: number, srcHeight: number, maxWidth: number, maxHeight: number) {
...
function computeCoverRect(srcWidth: number, srcHeight: number, dstWidth: number, dstHeight: number) {
...
async function createShareCardImage(sourceUrl: string): Promise<string> {
...
export async function prepareShareCardImage(sourceUrl: string): Promise<string> {
...
export function prefetchShareCardImage(sourceUrl: string) {
