...
exports.paramsToHeaders = paramsToHeaders;
...
function generateRequestParams(token, requestBody, apiPath, deviceID, hmacKey = 'jiadilinguang-hmac-secret-key-2024', sm4Key = '1234567890123456' // 16字节密钥（会自动转换为32字符十六进制）
) {
 ...
 */
function convertSM4KeyToHex(sm4Key) {
 ...
 * @returns hex编码的签名
 */
function generateTokenSignature(token, hmacKey) {
 ...
 * @returns sin字符串（32位）
 */
function generateSin(requestBody, sm4Key) {
    try {
        // 转换密钥为32字符十六进制格式
 ...
 */
function generateMD5Signature(sin, tokenSignature, apiPath, sm4Key) {
    try {
        // 转换密钥为32字符十六进制格式
        const hexKey = convertSM4KeyToHex(sm4Key);
 ...
 */
function generatePass(sin, md5Signature, deviceID, timestamp, sm4Key) {
    try {
        // 转换密钥为32字符十六进制格式
        const hexKey = convertSM4KeyToHex(sm4Key);
 ...
 * @returns tm字符串
 */
function generateTm(timestamp, apiPath, sm4Key) {
    try {
        // 转换密钥为32字符十六进制格式
        const hexKey = convertSM4KeyToHex(sm4Key);
        // 1. 将接口地址进行base64编码
