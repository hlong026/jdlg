...
function createProgressItems(days) {
    return days.map((day) => ({
        day,
...
function normalizeConsecutiveDays(data) {
    return Number(data?.consecutive_days ?? data?.consecutiveDays ?? 0);
}
function normalizeCheckedToday(data) {
    return Boolean(data?.checked_today ?? data?.checkedToday ?? false);
}
function getHeroBadgeSubText(consecutiveDays, checkedToday) {
...
Page({
     ...
     */
    data: {
        firstProgress: createProgressItems(FIRST_STAGE_DAYS),
        secondProgress: createProgressItems(SECOND_STAGE_DAYS),
     ...
     * 更新进度条状态
     */
    updateProgressStatus(consecutiveDays, checkedToday) {
    ...
    updateCheckinOverview(consecutiveDays, checkedToday) {
