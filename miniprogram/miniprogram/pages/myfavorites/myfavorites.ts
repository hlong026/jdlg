...
Page({
  ...
  mapFavoriteItem(data: any) {
    ...
    const creatorInfo = (data.creator_info || {}) as any;
      ...
      wx.request({
        ...
        success: (res) => {
          ...
          if (res.statusCode === 200 && body.code === 0) {
            ...
            resolve(Array.isArray(rawIds) ? rawIds.map((item: any) => Number(item || 0)).filter((item: number) => item > 0) : []);
            return;
          }
  ...
  async loadTemplateDetail(id: number) {
    try {
  ...
  async loadFavorites(options: { reset?: boolean; page?: number } = {}) {
    ...
    try {
      ...
      try {
        ...
        items = list.map((data: any) => this.mapFavoriteItem(data)).filter((item: any) => item.id > 0);
  ...
  async removeFavorite(e: any) {
    ...
    try {
      ...
      const allItems = (Array.isArray(this.data.allItems) ? this.data.allItems : []).filter((item: any) => item.id !== id);
      this.setData({
        allItems,
