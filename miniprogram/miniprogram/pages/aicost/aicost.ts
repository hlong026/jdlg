...
import { tryOpenCustomerServiceDirect } from '../../utils/enterpriseWechat';
...
function getRoofFactor(roofType: string): number {
  if (roofType === '平屋顶') {
    return 1;
  }
  if (roofType === '坡屋顶') {
...
function getBasementFactor(basementType: string): number {
...
function getDecorationFactor(decorationStyle: string): number {
...
function getFacadeFactor(facadeStyle: string): number {
  if (facadeStyle === '涂料外墙') {
    return 1;
  }
  if (facadeStyle === '真石漆') {
...
function getHeightFactor(floorHeight: number): number {
  if (floorHeight > 3.6) {
    return 1.08;
  }
  if (floorHeight > 3.3) {
...
function buildCostBreakdown(total: number): CostBreakdownItem[] {
  const items = [
...
Page({
  ...
  onBuildingAreaInput(e: any) {
    this.setData({ buildingArea: e.detail.value });
  },

  onFloorHeightInput(e: any) {
  ...
  onSelectOption(e: any) {
    ...
    const options = ((this.data as any)[optionsKey] || []) as string[];
  ...
  onEstimateCost() {
    ...
    const result = estimateTotalCost(
      ...
      floorHeight,
      this.data.roofType,
      this.data.basementType,
      this.data.decorationStyle,
      this.data.facadeStyle,
    );
