...
interface TemplateDownloadPayload {
  imageUrls: string[];
  downloadUrls: string[];
}

function mapDesignerCertStatusLabel(status: string): string {
  const value = String(status || '').trim();
  if (value === 'approved') return '平台认证';
...
Page({
  ...
  data: {
    ...
    templateDesc: '',
    mainImage: '', // 主要内容区域的主图
    imageList: [] as string[], // 多图列表，用于滑动预览
    // UI相关数据
    userInfo: {
      name: '匿名',
      avatar: '',
    ...
    localComments: [] as LocalCommentItem[],
  ...
  async loadCreatorCertificationStatus(userId: number) {
  ...
  queuePendingTemplateDownload(imageIndex?: number) {
    this.setData({
  ...
  async requestTemplateDownloadPayload(): Promise<TemplateDownloadPayload> {
    ...
    return {
      imageUrls: Array.isArray(result.image_urls) ? result.image_urls.map((item: any) => String(item || '').trim()).filter(Boolean) : [],
      downloadUrls: Array.isArray(result.download_urls) ? result.download_urls.map((item: any) => String(item || '').trim()).filter(Boolean) : [],
    };
  },

  buildTemplateDownloadProxyRequest(imageIndex: number, token: string) {
    ...
    const params = generateRequestParams(token, {}, apiPath, getCachedDeviceFingerprint() || '');
    return {
      url: `${API_BASE_URL}${apiPath}`,
      headers: paramsToHeaders(params),
    };
  },

  async performAuthorizedTemplateDownload(imageIndex: number = 0) {
    try {
      this.setData({ showEnterpriseWechatModal: false });
      ...
      const matchedRawIndex = imageUrls.findIndex((item: string) => item === currentUrl);
  ...
  normalizeCommentItem(item: any): LocalCommentItem {
  ...
  async loadComments() {
    ...
    try {
      ...
      const list = Array.isArray(response.list) ? response.list.map((item: any) => this.normalizeCommentItem(item)) : [];
      this.setData({
        localComments: list,
  ...
  async loadLikedState() {
    ...
    try {
      ...
      const ids = Array.isArray(data.ids) ? data.ids.map((item: any) => Number(item)) : [];
      this.setData({
