...
const DEFAULT_MAIN_TABS: TabItem[] = [
  { label: '场景', value: 'scene' },
  { label: '风格', value: 'style' },
  { label: '灵感', value: 'inspiration' },
];

const DEFAULT_SUB_TABS: TabItem[] = [
...
function normalizeSubTabs(raw: any): TabItem[] {
  if (!Array.isArray(raw)) {
    return [];
  }
  return raw
...
function getSubTabsByParent(subTabs: TabItem[], parent: string): TabItem[] {
...
Page({
  ...
  applyTabSelection(mainTab: string, subTab: string, thirdTab: string) {
  ...
  async loadWorkDetail(id: number) {
    ...
    try {
      ...
      this.applyTabSelection(
        String(detail.main_tab || ''),
        String(detail.sub_tab || ''),
        String(detail.third_tab || ''),
      );
      this.setData({
  ...
  uploadImage(filePath: string): Promise<string> {
    ...
    const params = generateRequestParams(token, '{}', apiPath, this.data.deviceId);
    ...
    return new Promise((resolve, reject) => {
      wx.uploadFile({
        ...
        success: (res) => {
          ...
          try {
            const body = JSON.parse(res.data) as any;
