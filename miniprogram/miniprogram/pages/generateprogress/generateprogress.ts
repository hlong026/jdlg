...
type ProgressTab = 'exterior' | 'interior';
...
Page({
  data: {
    ...
    entrySource: '',
    showSceneTabs: false,
    activeTab: 'exterior' as ProgressTab,
    taskNo: '',
  ...
  async getAuthHeaders(body: string, path: string) {
    const token = wx.getStorageSync('token');
    if (!token) {
      throw new Error('未登录');
    }
    const deviceID = getCachedDeviceFingerprint() || '';
    const params = generateRequestParams(token, body, path, deviceID);
    return paramsToHeaders(params);
  },

  async loadTaskStatus(taskNo: string, silent: boolean = false) {
    try {
      ...
      wx.request({
        ...
        success: (res: any) => {
          ...
          if (taskData.status === 'failed') {
            ...
            wx.showModal({
              title: '生成失败',
              content: sanitizeAIGenerationErrorMessage(taskData.error_message),
              showCancel: false,
              success: () => {
                this.onBack();
              }
        ...
        fail: () => {
          if (!silent) {
            wx.showToast({
              title: sanitizeAIGenerationErrorMessage('网络异常，请稍后重试'),
              icon: 'none'
            });
          } else {
            this.scheduleNextPoll(taskNo);
      ...
      if (!silent) {
        wx.showToast({
          title: sanitizeAIGenerationErrorMessage(error.message || '查询任务失败'),
          icon: 'none'
        });
      } else {
        this.scheduleNextPoll(taskNo);
