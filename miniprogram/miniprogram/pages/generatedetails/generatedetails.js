...
const PUBLISH_TEMPLATE_TOPICS = [
    { label: '场景', value: 'scene' },
    { label: '风格', value: 'style' },
    { label: '灵感', value: 'inspiration' },
];
const PUBLISH_TEMPLATE_SUB_TOPICS = [
...
Page({
    ...
    setTaskData(data) {
        ...
        if (data.error_message) {
            data.error_message = (0, aiError_1.sanitizeAIGenerationErrorMessage)(data.error_message);
        }
        else if (data.status === 'failed') {
            data.error_message = (0, aiError_1.sanitizeAIGenerationErrorMessage)('');
        }
        let durationText = '';
    ...
    buildProcessingStageText(status, progress) {
        if (status === 'pending') {
            if (progress < 20) {
    ...
    saveProgressState(percent, loadingStageText) {
    ...
    async getAuthHeaders(body = '', path = '/api/v1/miniprogram/ai/task/status') {
        const token = wx.getStorageSync('token');
        if (!token) {
            throw new Error('未登录');
        }
        const params = (0, parameter_1.generateRequestParams)(token, body, path);
        return (0, parameter_1.paramsToHeaders)(params);
    },
    async loadTaskDetail(taskNo, silent = false) {
    ...
    getSafeImageInfo(imageInfos, index) {
    ...
    async normalizeVideoFilePath(tempFilePath) {
    ...
    queuePendingEnterpriseWechatDownload(type, imageIndex) {
        const nextData = {
    ...
    async verifyEnterpriseWechatPhoneCode(code) {
    ...
    async onEnterpriseWechatPhoneAuth(e) {
    ...
    async requestOriginalDownloadUrl(imageIndex = 0) {
        const token = this.getSimpleToken();
        const taskNo = String(this.data.taskData.task_no || '').trim();
        if (!taskNo) {
    ...
    buildImageDownloadProxyUrl(imageIndex = 0) {
    ...
    async onDownloadVerifiedImage(imageIndex) {
